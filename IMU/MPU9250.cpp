...
bool cMPU9250::begin()
{
  ...
  SPI.setClockDivider( SPI_CLOCK_DIV128 ); // 108Mhz/128 = 0.8MHz
  SS_HIGH;

  delay( 100 );

  if( imu_spi_read(MPU9250_WHO_AM_I) == 0x71 )
  {
    bConnected = true;
    init();
    gyro_init();
...
void cMPU9250::init()
{
  ...
  //MPU9250 Reset
  imu_spi_write(MPU9250_PWR_MGMT_1, MPU9250_RESET);
  delay(100);
  //MPU9250 Set Clock Source
  imu_spi_write(MPU9250_PWR_MGMT_1,  MPU9250_CLOCK_PLLGYROZ);
  delay(1);


  //MPU9250 Set Interrupt
  imu_spi_write(MPU9250_INT_PIN_CFG,  MPU9250_INT_ANYRD_2CLEAR);
  delay(1);
  imu_spi_write(MPU9250_INT_ENABLE, 0xff);
  delay(1);
  //MPU9250 Set Sensors
  imu_spi_write(MPU9250_PWR_MGMT_2, MPU9250_XYZ_GYRO & MPU9250_XYZ_ACCEL);
  delay(1);
  //MPU9250 Set SampleRate
  //SAMPLE_RATE = Internal_Sample_Rate / (1 + SMPLRT_DIV)
  imu_spi_write(MPU9250_SMPLRT_DIV, SMPLRT_DIV);
  delay(1);
  //MPU9250 Set Full Scale Gyro Range
  //Fchoice_b[1:0] = [00] enable DLPF
  imu_spi_write(MPU9250_GYRO_CONFIG, (MPU9250_FSR_2000DPS << 3));
  delay(1);
  //MPU9250 Set Full Scale Accel Range PS:2G
  imu_spi_write( MPU9250_ACCEL_CONFIG, (MPU9250_FSR_2G << 3));
  delay(1);
  //MPU9250 Set Accel DLPF
  data = imu_spi_read(MPU9250_ACCEL_CONFIG2);
  data |= MPU9250_ACCEL_DLPF_41HZ;
  delay(1);
  imu_spi_write(MPU9250_ACCEL_CONFIG2, data);
  delay(1);
  //MPU9250 Set Gyro DLPF
  imu_spi_write( MPU9250_CONFIG, MPU9250_GYRO_DLPF_41HZ);
  delay(1);



  //MPU9250 Set SPI Mode
  state = imu_spi_read(MPU9250_USER_CTRL);
  delay(1);
  imu_spi_write(MPU9250_USER_CTRL, state | MPU9250_I2C_IF_DIS);
  delay(1);
  state = imu_spi_read(MPU9250_USER_CTRL);
  delay(1);
  imu_spi_write(MPU9250_USER_CTRL, state | MPU9250_I2C_MST_EN);
  delay(1);
  //////////////////////////////////////////////////////////////////////////
  //AK8963 Setup
  //reset AK8963
  ...
  delay(1);
  //
  imu_spi_write( MPU9250_I2C_MST_CTRL, 0x5D);
  delay(1);
  imu_spi_write(MPU9250_I2C_SLV0_ADDR, MPU9250_AK8963_I2C_ADDR | MPU9250_I2C_READ);
  delay(1);
  imu_spi_write(MPU9250_I2C_SLV0_REG, MPU9250_AK8963_ST1);
  delay(1);
  imu_spi_write( MPU9250_I2C_SLV0_CTRL, 0x88);
  delay(1);
  //
  ...
  //
  imu_spi_write(MPU9250_I2C_SLV4_CTRL, 0x09);
  delay(1);
  //
  imu_spi_write( MPU9250_I2C_MST_DELAY_CTRL, 0x81);
  delay(100);
}
...
void cMPU9250::gyro_get_adc( void )
{
  ...
  if( bConnected == true )
  {
    imu_spi_reads( MPU9250_GYRO_XOUT_H, 6, rawADC );
...
void cMPU9250::acc_get_adc( void )
{
  ...
  if( bConnected == true )
  {
    imu_spi_reads( MPU9250_ACCEL_XOUT_H, 6, rawADC );
...
void cMPU9250::mag_get_adc( void )
{
  ...
  if( bConnected == true )
  {
    imu_spi_reads( MPU9250_EXT_SENS_DATA_00, 8, data);
